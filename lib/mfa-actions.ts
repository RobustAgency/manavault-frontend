...
export async function enrollMFA() {
    ...
    if (userError || !user) {
        return { success: false, message: userError?.message || "Not authenticated" } as const;
    }
    ...
    if (!data) {
        ...
        return { success: false, message: "No data returned from enrollment" } as const;
    }
    ...
    // Try multiple possible paths for QR code and secret
    const qrCode = data?.totp?.qr_code || data?.qr_code || data?.qrcode || '';
    ...
    if (!qrCode || !secret) {
        ...
        return { 
            success: false, 
            message: `Failed to get QR code or secret from enrollment. QR Code: ${qrCode ? 'present' : 'missing'}, Secret: ${secret ? 'present' : 'missing'}. Please check the server logs.` 
        } as const;
    }
